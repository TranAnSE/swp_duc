...
<html>
    ...
    <body>
        ...
        <div class="container">
            <h2><i class="fas fa-clipboard-list"></i> Quản lý bài Test - Admin</h2>
                            ...
                            <select class="form-control select2-dropdown" id="courseId" name="courseId" data-theme="bootstrap4">
                                ...
                                <c:forEach var="course" items="${courses}">
                                    <option value="${course.course_id}" ${selectedCourseId == course.course_id ? 'selected' : ''}>
                                        ${course.course_title}
                                        <c:if test="${not empty course.subject_name}">
                                            - ${course.subject_name}
                                        </c:if>
                                        <c:if test="${not empty course.grade_name}">
                                            (${course.grade_name})
                                        </c:if>
                                    </option>
                                </c:forEach>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover">
                    <thead>
                        <tr>
                            <th width="6%">ID</th>
                            <th width="20%">Tên Test</th>
                            <th width="25%">Mô tả</th>
                            <th width="8%">Loại</th>
                            <th width="15%">Khóa học</th>
                            <th width="6%">Câu hỏi</th>
                            ...
                            <c:when test="${not empty testList}">
                                <c:forEach var="test" items="${testList}">
                                    <tr>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${fn:length(test.test_description) > 80}">
                                                    ${fn:substring(test.test_description, 0, 80)}...
                                                </c:when>
                                                <c:otherwise>
                                        ...
                                        <td>
                                            <small>${test.created_by_name}</small>
                                        </td>
                                        <td>
