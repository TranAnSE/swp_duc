...
public class GoogleCloudStorageService {
    ...
    /**
     * Upload image lên Google Cloud Storage
     *
     * @param fileContent Nội dung file image
     * @param fileName Tên file gốc
     * @param contentType Loại nội dung (MIME type)
     * @return URL của image đã upload
     */
    public String uploadImage(InputStream fileContent, String fileName, String contentType) {
        try {
            // Tạo tên file duy nhất để tránh trùng lặp
            String uniqueFileName = generateUniqueFileName(fileName);
            String objectPath = "images/thumbnails/" + uniqueFileName;
            ...
            if (responseCode >= 200 && responseCode < 300) {
                // Upload thành công
                logger.info("Successfully uploaded image: " + uniqueFileName);
                return "https://storage.googleapis.com/" + bucketName + "/" + objectPath;
            } else {
                // Upload thất bại
                try (InputStream errorStream = connection.getErrorStream()) {
                    ...
                    String errorResponse = new String(errorBuffer.toByteArray());
                    logger.severe("Failed to upload image. Response code: " + responseCode + ", Error: " + errorResponse);
                }
                throw new IOException("Failed to upload image. Response code: " + responseCode);
            }

        } catch (Exception e) {
            logger.log(Level.SEVERE, "Error uploading image: " + fileName, e);
    ...
    public boolean deleteImage(String imageUrl) {
        try {
            ...
            if (!imageUrl.startsWith(prefix)) {
                logger.warning("Invalid image URL format: " + imageUrl);
                return false;
            }
            ...
            int responseCode = connection.getResponseCode();
            if (responseCode >= 200 && responseCode < 300) {
                logger.info("Successfully deleted image: " + objectPath);
                return true;
            } else {
                logger.warning("Failed to delete image. Response code: " + responseCode);
                return false;
            }
